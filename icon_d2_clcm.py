...
FILE_LAST_HOUR = "ultima_ora_icond2_clcm.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per CLCM.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    try:
        ...
        print(f"📸 Album Telegram CLCM inviato ({len(file_paths)} mappe).")
    except Exception as e:
...
def genera_album_clcm(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        ...
        for h in ore_list:
            try:
                print(f"  ⬇️  Elaborazione CLCM H={h}...")
                curr_clcm = scarica_step_clcm(dt_run_utc, h)
                clcm_mean_xr = curr_clcm.mean(dim="eps")

                lat_vals = clcm_mean_xr['latitude'].values
                lon_vals = clcm_mean_xr['longitude'].values
                mean_vals = clcm_mean_xr.values
                ...
                if np.any(mask):
                    ...
                    cbar.set_label("Nuvolosità Media Media (%)", fontweight='bold')
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: Nuvolosità Media Media (%)\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Nuvolosità Media ICON-D2 per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
        genera_album_clcm(dt_run_utc, nome_run)
    else:
