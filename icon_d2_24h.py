...
FILE_LAST_HOUR, RUN_DURATION, START_DELAY = "ultima_ora_icond2_24h.txt", 48, 0
...
def invia_album_telegram(file_paths: list, caption: str):
    token, chat_id, thread_id = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_4")
...
def genera_album_24h(dt_run_utc: datetime, nome_run: str):
    ...
    for idx, (date_str, ore_list) in enumerate(giorni.items(), start=1):
        min_h, max_h = min(ore_list), max(ore_list)
        print(f"\nGenerazione accumulo 24h per {date_str}...")
        try:
            if max_h not in cache_tot: cache_tot[max_h] = scarica_step_precipitazione(dt_run_utc, max_h)
            tot_end = cache_tot[max_h]
            if min_h > 1:
                if (min_h - 1) not in cache_tot: cache_tot[min_h - 1] = scarica_step_precipitazione(dt_run_utc, min_h - 1)
                prec_finestra = tot_end.copy(data=np.maximum(0, tot_end.values - cache_tot[min_h - 1].values))
            else: prec_finestra = tot_end

            mean_xr = prec_finestra.mean(dim="eps")
            ...
            if np.max(mean_crop) >= my_levels[0]:
                ...
                cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("Precipitazione Cumulata Media (mm)", fontweight='bold')
            else:
                sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([]) 
                cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("Precipitazione Cumulata Media (mm)", fontweight='bold')
            ...
            start_local, end_local = (dt_run_utc + timedelta(hours=min_h-1)).astimezone(rome_tz), (dt_run_utc + timedelta(hours=max_h)).astimezone(rome_tz)
            plt.title(f"ICON-D2 EPS - Precipitazione Cumulata Media (mm)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {start_local.strftime('%H:%M del %d/%m/%Y')} - {end_local.strftime('%H:%M del %d/%m/%Y')}", fontweight='bold')
    ...
    if percorsi_foto:
        invia_album_telegram(percorsi_foto, f"ICON-D2 EPS: Precipitazione Cumulata Giornaliera (mm)\nRun {nome_run} ({dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')})")
...
if __name__ == "__main__":
    ...
    if data:
        ...
        if is_new: genera_album_24h(dt_run_utc, nome_run)
