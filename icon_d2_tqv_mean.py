...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, windows in blocchi.items():
        ...
        for w in windows:
            ...
            h_end = w[-1]
            try:
                print(f"  💧 Elaborazione TQV H={h_start}-{h_end}...")
                
                datasets = []
                for hr in w:
                    datasets.append(scarica_step_tqv(dt_run_utc, hr))
                ...
                title = f"ICON-D2 EPS - Acqua Precipitabile Media (TQV) [Max 3h]\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: Acqua Precipitabile Media [Max 3h]\n{block_name}\nRun {nome_run}"
