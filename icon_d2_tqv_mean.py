...
# File di controllo dedicato a TQV per evitare sovrapposizioni
FILE_LAST_HOUR = "ultima_ora_icond2_tqv.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per TQV (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def scarica_step_tqv(dt_run_utc, h_step, max_retries=3):
    """
    Scarica il parametro Acqua Precipitabile (TQV) sulla GRIGLIA NATIVA ICOSAEDRALE.
    """
    ...
    url_tqv = f"https://opendata.dwd.de/weather/nwp/icon-d2-eps/grib/{run_hour}/tqv/icon-d2-eps_germany_icosahedral_single-level_{date_hour}_{step_str}_2d_tqv.grib2.bz2"
    ...
    p_tqv = _download_one(url_tqv)
    ds_tqv = earthkit.data.from_source("file", p_tqv).to_xarray()

    if 'member' in ds_tqv.dims: ds_tqv = ds_tqv.rename({'member': 'eps'})
    elif 'number' in ds_tqv.dims: ds_tqv = ds_tqv.rename({'number': 'eps'})

    tqv_var = list(ds_tqv.data_vars)[0]
    tqv_data = ds_tqv[tqv_var].compute()
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_1134")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    # Scala dell'Acqua Precipitabile fornita (11 livelli, 10 colori)
    my_levels = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 60]
    my_colors = ["#ffffcc", "#c7e9b4", "#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#253494", "#081d58", "#810f7c", "#4d004b"]
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album TQV: {block_name}")
        percorsi_foto = []

        for h in ore_list:
            try:
                print(f"  💧 Elaborazione TQV H={h}...")
                curr_tqv = scarica_step_tqv(dt_run_utc, h)

                # --- Calcolo della Media degli Scenari (Ensemble Mean) ---
                tqv_mean_xr = curr_tqv.mean(dim="eps")
                
                lat_vals = tqv_mean_xr['latitude'].values
                lon_vals = tqv_mean_xr['longitude'].values
                mean_vals = tqv_mean_xr.values
                ...
                # Filtro i valori per plottare solo a partire dal valore minimo della scala
                mask = mean_vals >= 5
                ...
                # Il TQV (Total column integrated water vapour) è in kg/m², che corrisponde ai mm.
                cbar.set_label("Acqua Precipitabile Media (kg/m² o mm)", fontweight='bold')
                ...
                title = f"ICON-D2 EPS - Acqua Precipitabile Media (TQV)\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {str_valida}"
                plt.title(title, fontweight='bold')

                filename = f"tqv_{h}.png"
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: Acqua Precipitabile Media\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Acqua Precipitabile Media per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
