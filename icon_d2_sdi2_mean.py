...
# File di controllo dedicato a SDI2 per evitare sovrapposizioni
FILE_LAST_HOUR = "ultima_ora_icond2_sdi2.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per SDI2 (Ultimo blocco: {ultima_ora_valida_str}).")
            return False, "", None
...
def scarica_step_sdi2(dt_run_utc, h_step, max_retries=3):
    """
    Scarica il parametro SDI 2 sulla GRIGLIA NATIVA ICOSAEDRALE.
    """
    ...
    p_sdi = _download_one(url_sdi)
    ds_sdi = earthkit.data.from_source("file", p_sdi).to_xarray()

    if 'member' in ds_sdi.dims: ds_sdi = ds_sdi.rename({'member': 'eps'})
    elif 'number' in ds_sdi.dims: ds_sdi = ds_sdi.rename({'number': 'eps'})

    sdi_var = list(ds_sdi.data_vars)[0]
    sdi_data = ds_sdi[sdi_var].compute()
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        ...
        for h in ore_list:
            try:
                print(f"  🌪️ Elaborazione SDI 2 H={h}...")
                curr_sdi = scarica_step_sdi2(dt_run_utc, h)
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: SDI 2 Medio\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione SDI 2 Medio per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
