...
FILE_LAST_HOUR, RUN_DURATION, START_DELAY = "ultima_ora_icond2_sdi2.txt", 48, 0
...
def scarica_step_sdi2(dt_run_utc, h_step, max_retries=3):
    run_hour, date_hour, step_str = f"{dt_run_utc.hour:02d}", dt_run_utc.strftime('%Y%m%d%H'), f"{h_step:03d}"
    url = f"https://opendata.dwd.de/weather/nwp/icon-d2-eps/grib/{run_hour}/sdi_2/icon-d2-eps_germany_icosahedral_single-level_{date_hour}_{step_str}_2d_sdi_2.grib2.bz2"
...
def invia_album_telegram(file_paths: list, caption: str):
    token, chat_id, thread_id = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_883")
...
def genera_album_orari(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, windows in blocchi.items():
        print(f"\nGenerazione album SDI2: {block_name}")
        percorsi_foto = []
        for w in windows:
            h_start, h_end = w[0], w[-1]
            try:
                datasets = [scarica_step_sdi2(dt_run_utc, hr) for hr in w]
                ...
                if np.max(mean_crop) >= my_levels[0]:
                    ...
                    cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("SDI 2 Medio (Adimensionale)", fontweight='bold')
                else:
                    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([]) 
                    cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("SDI 2 Medio (Adimensionale)", fontweight='bold')
        ...
        if percorsi_foto:
            invia_album_telegram(percorsi_foto, f"ICON-D2 EPS: SDI 2 Medio [Max 3h]\n{block_name}\nRun {nome_run}")
