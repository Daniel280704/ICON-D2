...
FILE_LAST_HOUR = "ultima_ora_icond2_mlcin.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per MLCIN.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    thread_id = os.getenv("TELEGRAM_THREAD_ID_17")
    ...
    try:
        ...
        print(f"📸 Album Telegram MLCIN inviato ({len(file_paths)} mappe).")
    except Exception as e:
...
def genera_album_mlcin(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        ...
        for h in ore_list:
            try:
                print(f"  ⬇️  Elaborazione MLCIN H={h}...")
                curr_cin = scarica_step_mlcin(dt_run_utc, h)
                cin_mean_xr = curr_cin.mean(dim="eps")

                lat_vals = cin_mean_xr['latitude'].values
                lon_vals = cin_mean_xr['longitude'].values
                mean_vals = cin_mean_xr.values
                ...
                if np.any(mask):
                    ...
                    cbar.set_label("MLCIN Medio (J/kg)", fontweight='bold')
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: MLCIN Medio (J/kg)\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione MLCIN ICON-D2 per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
        genera_album_mlcin(dt_run_utc, nome_run)
    else:
