...
def genera_album_mlcin(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, windows in blocchi.items():
        ...
        for w in windows:
            ...
            h_end = w[-1]
            try:
                print(f"  ⬇️  Elaborazione MLCIN H={h_start}-{h_end}...")
                
                datasets = []
                for hr in w:
                    datasets.append(scarica_step_mlcin(dt_run_utc, hr))
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: MLCIN Medio (J/kg) [Max 3h]\n{block_name}\nRun {nome_run}"
