...
FILE_LAST_HOUR, RUN_DURATION, START_DELAY = "ultima_ora_icond2_mlcin.txt", 48, 0
...
def scarica_step_mlcin(dt_run_utc, h_step, max_retries=3):
    run_hour, date_hour, step_str = f"{dt_run_utc.hour:02d}", dt_run_utc.strftime('%Y%m%d%H'), f"{h_step:03d}"
    url = f"https://opendata.dwd.de/weather/nwp/icon-d2-eps/grib/{run_hour}/cin_ml/icon-d2-eps_germany_icosahedral_single-level_{date_hour}_{step_str}_2d_cin_ml.grib2.bz2"
...
def invia_album_telegram(file_paths: list, caption: str):
    token, chat_id, thread_id = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_17")
...
def raggruppa_in_blocchi(dt_run_local: datetime) -> dict:
    ...
    for h_start in range(1, 49, 3): 
        date_str = (dt_run_local + timedelta(hours=h_start)).strftime("%d-%m-%Y")
        key = f"Giornata del {date_str}"
        if key not in blocchi: blocchi[key] = []
        blocchi[key].append(range(h_start, min(h_start + 2, 48) + 1))
    return blocchi

def genera_album_mlcin(dt_run_utc, nome_run):
    ...
    my_colors = ["#e0f3f8", "#abd9e9", "#74add1", "#4575b4", "#313695", "#fee090", "#fdae61", "#f46d43", "#d73027", "#a50026", "#8e0152", "#c51b7d", "#de77ae", "#f1b6da", "#fde0ef"]
    ...
    for block_name, windows in blocchi.items():
        print(f"\nGenerazione album MLCIN medio: {block_name}")
        percorsi_foto = []
        for w in windows:
            h_start, h_end = w[0], w[-1]
            try:
                datasets = [scarica_step_mlcin(dt_run_utc, hr) for hr in w]
                mean_xr = xr.concat(datasets, dim="time").max(dim="time").mean(dim="eps")
                ...
                if np.max(mean_crop) >= my_levels[0]:
                    ...
                    cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("MLCIN Medio (J/kg)", fontweight='bold')
                else:
                    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([]) 
                    cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("MLCIN Medio (J/kg)", fontweight='bold')
                ...
                start_local, end_local = dt_run_local + timedelta(hours=h_start-1), dt_run_local + timedelta(hours=h_end)
                plt.title(f"ICON-D2 EPS - MLCIN Medio (J/kg) [Max 3h]\nRun: {dt_run_utc.strftime('%d/%m/%Y %H:%M UTC')} | {start_local.strftime('%H:%M')} - {end_local.strftime('%H:%M del %d/%m')} (+{h_start}-{h_end}h)", fontweight='bold')
                filename = f"mlcin_{h_start}_{h_end}.png"
                plt.savefig(filename, dpi=200, bbox_inches='tight'); plt.close(fig); percorsi_foto.append(filename)
            except Exception as e: print(f"  ❌ Errore ore {h_start}-{h_end}: {e}")

        if percorsi_foto:
            invia_album_telegram(percorsi_foto, f"ICON-D2 EPS: MLCIN Medio (J/kg) [Max 3h]\n{block_name}\nRun {nome_run}")
...
if __name__ == "__main__":
    ...
    if data:
        ...
        if is_new: genera_album_mlcin(dt_run_utc, nome_run)
