...
FILE_LAST_HOUR = "ultima_ora_icond2_vmax.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per VMAX_10M.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    try:
        ...
        print(f"📸 Album Telegram VMAX_10M inviato ({len(file_paths)} mappe).")
    except Exception as e:
...
def genera_album_vmax(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        ...
        for h in ore_list:
            try:
                print(f"  ⬇️  Elaborazione VMAX_10M H={h}...")
                curr_vmax = scarica_step_vmax(dt_run_utc, h)
                vmax_mean_xr = curr_vmax.mean(dim="eps")

                # Conversione da m/s a km/h
                vmax_mean_kmh = vmax_mean_xr * 3.6

                lat_vals = vmax_mean_kmh['latitude'].values
                lon_vals = vmax_mean_kmh['longitude'].values
                mean_vals = vmax_mean_kmh.values
                ...
                # Mostriamo i valori >= 10 km/h per evitare di coprire lo sfondo con raffiche irrilevanti
                mask = mean_vals >= 10
                ...
                if np.any(mask):
                    ...
                    cbar.set_label("Raffica Massima Media a 10m (km/h)", fontweight='bold')
        ...
        if percorsi_foto:
            caption_album = f"ICON-D2 EPS: Raffica Massima Media (km/h)\n{block_name}\nRun {nome_run}"
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione Raffica Massima ICON-D2 per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
        genera_album_vmax(dt_run_utc, nome_run)
    else:
