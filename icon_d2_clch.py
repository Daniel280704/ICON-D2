...
FILE_LAST_HOUR, RUN_DURATION, START_DELAY = "ultima_ora_icond2_clch.txt", 48, 0
...
def scarica_step_clch(dt_run_utc, h_step, max_retries=3):
    run_hour, date_hour, step_str = f"{dt_run_utc.hour:02d}", dt_run_utc.strftime('%Y%m%d%H'), f"{h_step:03d}"
    url = f"https://opendata.dwd.de/weather/nwp/icon-d2-eps/grib/{run_hour}/clch/icon-d2-eps_germany_icosahedral_single-level_{date_hour}_{step_str}_2d_clch.grib2.bz2"
...
def invia_album_telegram(file_paths: list, caption: str):
    token, chat_id, thread_id = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_8")
...
def genera_album_clch(dt_run_utc: datetime, nome_run: str):
    ...
    for block_name, ore_list in blocchi.items():
        print(f"\nGenerazione album Nuvolosità Alta media: {block_name}")
        percorsi_foto = []
        for h in ore_list:
            try:
                curr_clch = scarica_step_clch(dt_run_utc, h)
                clch_mean_xr = curr_clch.mean(dim="eps")

                # --- SCHIACCIASASSI E RITAGLIO ---
                lat_vals = clch_mean_xr['latitude'].values.flatten()
                lon_vals = clch_mean_xr['longitude'].values.flatten()
                mean_vals = clch_mean_xr.values.flatten()
                ...
                if np.nanmax(mean_crop) >= my_levels[1]: # Partiamo da my_levels[1] (10) come nel tuo mask originale
                    ...
                    cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("Nuvolosità Alta Media (%)", fontweight='bold')
                else:
                    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([]) 
                    cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("Nuvolosità Alta Media (%)", fontweight='bold')
        ...
        if percorsi_foto:
            invia_album_telegram(percorsi_foto, f"ICON-D2 EPS: Nuvolosità Alta Media (%)\n{block_name}\nRun {nome_run}")
...
if __name__ == "__main__":
    ...
    if data:
        ...
        if is_new: genera_album_clch(dt_run_utc, nome_run)
