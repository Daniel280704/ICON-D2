...
FILE_LAST_HOUR = "ultima_ora_icond2_mlcape.txt" 
RUN_DURATION = 48 
START_DELAY = 0
...
def estrai_limiti_run(hourly_data: dict, ref_param: str, utc_offset_sec: int) -> tuple[bool, str, datetime]:
    ...
    if os.path.exists(FILE_LAST_HOUR):
        ...
        if ultima_ora_valida_str <= ultima_ora_salvata:
            print(f"✅ Run ICON-D2 EPS {nome_run} già elaborato per MLCAPE.")
            return False, "", None
...
def invia_album_telegram(file_paths: list, caption: str):
    ...
    try:
        ...
        print(f"📸 Album Telegram MLCAPE inviato ({len(file_paths)} mappe).")
    except Exception as e:
        ...
        for h in ore_list:
            try:
                ...
                if np.any(mask):
                    ...
                    cbar.set_label("MLCAPE Medio (J/kg)", fontweight='bold')
...
def main():
    ...
    if is_new:
        print(f"🚀 Lancio generazione MLCAPE ICON-D2 per il RUN {nome_run} ({dt_run_utc.strftime('%Y-%m-%d %H:%M')})")
