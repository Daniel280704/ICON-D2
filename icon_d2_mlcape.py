...
FILE_LAST_HOUR, RUN_DURATION, START_DELAY = "ultima_ora_icond2_mlcape.txt", 48, 0
...
def scarica_step_mlcape(dt_run_utc, h_step, max_retries=3):
    run_hour, date_hour, step_str = f"{dt_run_utc.hour:02d}", dt_run_utc.strftime('%Y%m%d%H'), f"{h_step:03d}"
    url = f"https://opendata.dwd.de/weather/nwp/icon-d2-eps/grib/{run_hour}/cape_ml/icon-d2-eps_germany_icosahedral_single-level_{date_hour}_{step_str}_2d_cape_ml.grib2.bz2"
...
def invia_album_telegram(file_paths: list, caption: str):
    token, chat_id, thread_id = os.getenv("TELEGRAM_TOKEN"), os.getenv("TELEGRAM_CHAT_ID"), os.getenv("TELEGRAM_THREAD_ID_16")
...
def genera_album_mlcape(dt_run_utc, nome_run):
    ...
    my_colors = ["#4575b4", "#74add1", "#abd9e9", "#e0f3f8", "#ffffbf", "#fee090", "#fdae61", "#f46d43", "#d73027", "#c7192b", "#b30000", "#8b0000", "#660000", "#440000", "#220000"]
    ...
    for block_name, windows in blocchi.items():
        ...
        for w in windows:
            ...
            try:
                datasets = [scarica_step_mlcape(dt_run_utc, hr) for hr in w]
                ...
                if np.max(mean_crop) >= my_levels[0]:
                    ...
                    cbar = plt.colorbar(cf, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("MLCAPE Medio (J/kg)", fontweight='bold')
                else:
                    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([]) 
                    cbar = plt.colorbar(sm, ax=ax, orientation='horizontal', shrink=0.7, pad=0.05); cbar.set_label("MLCAPE Medio (J/kg)", fontweight='bold')
        ...
        if percorsi_foto:
            invia_album_telegram(percorsi_foto, f"ICON-D2 EPS: MLCAPE Medio (J/kg) [Max 3h]\n{block_name}\nRun {nome_run}")
...
if __name__ == "__main__":
    ...
    if data:
        ...
        if is_new: genera_album_mlcape(dt_run_utc, nome_run)
